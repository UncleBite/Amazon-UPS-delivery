...
#world_host = "server"
amazon_host = "vcm-8250.vm.duke.edu"
...
def connect_db():
    while 1:
        try:
            ...
            db_cur.execute( "CREATE TABLE IF NOT EXISTS world_ack(seqnum INT PRIMARY KEY, message json )")
            db_cur.execute( "CREATE TABLE IF NOT EXISTS amazon_ack(seqnum INT PRIMARY KEY, message json )")
            break
...
def main():
    ...
    thread2.start()
    thread3 = threading.Thread(target=send_unack_msg_to_amazon,args=(amazon_fd,))
    thread3.start()
    thread4 = threading.Thread(target=send_unack_msg_to_world,args=(world_fd,))
    thread4.start()
