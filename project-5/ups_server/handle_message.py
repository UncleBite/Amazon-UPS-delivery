...
def truckid_selector():
    ...
    row = db_cur.fetchone()
    if row is not None:
        db_cur.execute("UPDATE truck SET status = E WHERE truckid = {};").format(row[2])
        db_conn.commit()
        return row[2]
...
def completions_handler(completions, world_id, amazon_fd, world_fd):
    ...
    db_cur.execute("""INSERT INTO amazon_ack (seqnum,message) VALUES(%s,%s);""",(seqnum,json.dumps(json_msg)))
    db_conn.commit()
...
def send_email(world_id, amazon_fd, world_fd, package_id):
    host = "smtp.gmail.com"
    port = 465
    sender = "miniups104@gmail.com"
    pwd = "abc123456789!"
    ...
    try:
        ...
        print(error)
    db_cur.execute("select email, name, location_x, location_y from auth_user, package where"
                   "packageid = '"+str(package_id) +"' and auth_user.username = package.name")
    rows = db_cur.fetchall()
    row = rows[0]
    receiver = row[0]
    username = row[1]
...
def orderplaced_handler(orderplaced_handle_list, world_id, amazon_fd, world_fd, truck_list):
    ...
    json_msg = pb2json(UCommu)
    db_cur.execute("""INSERT INTO amazon_ack (seqnum,message) VALUES(%s,%s);""",(seqnum1,json.dumps(json_msg)))
    db_conn.commit()    
    send_to_amazon(UCommu, amazon_fd)
    ...
    db_cur.execute("""INSERT INTO world_ack (seqnum,message) VALUES(%s,%s);""",(seqnum2,json.dumps(json_msg2)))
    db_conn.commit()
...
def loadingfinished_handler(loadingfinished_handle_list, world_id, amazon_fd, world_fd):
    ...
    db_cur.execute("""INSERT INTO world_ack (seqnum,message) VALUES(%s,%s);""",(seqnum,json.dumps(json_msg)))
    db_conn.commit()
    send_to_world(UCommands, world_fd)
